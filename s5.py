...
stars: list[Star] = [make_star() for _ in range(n_dots)]


mouse_line_radius = int(150 * width_ratio)
stars_line_radius = int(150 * width_ratio)
...
while True:
    ...
    # bg.fill(screen, dt)
    # screen.fill((25, 0, 50))
    mouse_real_pos = pg.mouse.get_pos()
    old_positions.append(mouse_real_pos)
    for i, star in enumerate(stars):
        ...
        mouse_pos = (old_xpos, old_ypos)
        x_distance = star.xpos+star.w//2 - mouse_pos[0]
        y_distance = star.ypos+star.h//2 - mouse_pos[1]
